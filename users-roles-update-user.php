...
function update_user_add_toplevel_menu() {
	...
	add_menu_page(
		esc_html__('Users and Roles: Update User', 'myplugin'),
		esc_html__('Update User', 'myplugin'),
		'manage_options',
		'myplugin',
...
function update_user_display_settings_page() {
	...
	<div class="wrap">
		...
		<form method="post">
			<h3><?php esc_html_e( 'Update User', 'myplugin' ); ?></h3>
			<p>
				<label for="email">
			...
			<p>
				<label for="display-name">
					<?php esc_html_e( 'Enter a new Display Name for this user:', 'myplugin' ); ?>
				</label><br />
				<input class="regular-text" type="text" size="40" name="display-name" id="display-name">
			</p>
			<p>
				<label for="user-url">
					<?php esc_html_e( 'Enter a new Website URL for this user:', 'myplugin' ); ?>
				</label><br />
			...
			<input type="submit" class="button button-primary" value="<?php esc_html_e( 'Update User', 'myplugin' ); ?>">
		</form>
	</div>
